        ...
        public async Task Consume(ConsumeContext<SpotSubmittedEvent> context)
        {
            ...
            try
            {
                var submission = new Submission()
                {
                    ...
                    Accessibility = string.IsNullOrEmpty(message.Accessibility) ? null : Enum.Parse<Accessibility>(message.Accessibility),
                    Condition = string.IsNullOrEmpty(message.Condition) ? null : Enum.Parse<Condition>(message.Condition),
                    SkillLevel = string.IsNullOrEmpty(message.SkillLevel) ? null : Enum.Parse<SkillLevel>(message.SkillLevel),
